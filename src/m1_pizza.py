...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(250.0, 130.0),  # All numbers are approximate.
                rg.Point(200.0, 180.0),
                rg.Point(150.0, 130.0),
                rg.Point(200.0, 80.0)]
    circle = rg.Circle(rg.Point(200, 130), 50)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for _ in range(number_of_slices):
        ...
        line = rg.Line(circle.center, points[_])
...
def run_test_polygon():
    ...
    # Test 3:
    title = 'POLYGON test 3: 5 segments with thick black lines on lime green circle.'
    window = rg.RoseWindow(400, 450, title)

    circle = rg.Circle(rg.Point(200, 225), 150)
    circle.outline_thickness = 1
    circle.fill_color = 'lime green'
    polygon(window, circle, 5, 'black', 10)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for _ in range(number_of_segments - 1):
        ...
        line = rg.Line(points[_], points[_ + 1])
        line_end = rg.Line(points[number_of_segments - 1], points[0])
        line.color = color
        line_end.color = color
        line.thickness = thickness
        line_end.thickness = thickness
        line.attach_to(window)
        line_end.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4: 20 red lines on white circle, hops = 5'
    window = rg.RoseWindow(300, 460, title)

    circle = rg.Circle(rg.Point(150, 250), 100)
    fancy_polygon(window, circle, 20, 5, 'red', 3)
    window.close_on_mouse_click()
